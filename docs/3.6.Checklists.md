...
A seguir, temos um template de checklist de Pull Request voltado para Acessibilidade Web, que você pode adaptar e personalizar de acordo com:
- Necessidades do projeto
- Nível de maturidade dos desenvolvedores ou desenvolvedor
- Ferramentas utilizadas (Ex: se seu projeto apenas utiliza linters para verifica;'ao de acesibilidade n'ao faz sentido colocar outras ferramentas al[em disso]. alem do mais vc pode adaptar para as ferramentas especificas. por ex se usar o eslint-plugin-jsx-a11y pode citar ele diretamente ou criterios especificos dele como mapeamente do componentes por ex)
. Este template combina pontos e ferramentas de verificação baseados nas ferramentas e processos que este guia propoe. todas elas estao baseadas na wcag. -->
